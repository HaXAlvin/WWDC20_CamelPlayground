...
class mygameScene:SKScene{
    ...
    var sd_lamp_switch = SKAudioNode(fileNamed: "/audio/lamp_switch.mp3")
    var sd_air_con = SKAudioNode(fileNamed: "/audio/air_conditioner.mp3")
    var sd_waterer = SKAudioNode(fileNamed: "/audio/waterer.mp3")
    var sd_shower = SKAudioNode(fileNamed: "/audio/shower.mp3")
    var sd_ref = SKAudioNode(fileNamed: "/audio/refrigerator.mp3")
    var sd_fire_down = SKAudioNode(fileNamed: "/audio/fire_down.wav")
    var sd_fire = SKAudioNode(fileNamed: "/audio/fire.mp3")
    override init(size:CGSize) {
        ...
        sd_lamp_switch.autoplayLooped = false
        self.addChild(sd_lamp_switch)
        sd_air_con.run(SKAction.stop())
        self.addChild(sd_air_con)
        
        sd_waterer.autoplayLooped = false
        self.addChild(sd_waterer)
        
        sd_shower.run(SKAction.changeVolume(to: 0.6, duration: 0))
        sd_shower.run(SKAction.stop())
        self.addChild(sd_shower)
        
        sd_ref.run(SKAction.stop())
        self.addChild(sd_ref)
    
        
        sd_fire_down.autoplayLooped = false
        self.addChild(sd_fire_down)
        
        sd_fire.autoplayLooped = false
        self.addChild(sd_fire)
        
        let backgroundSound = SKAudioNode(fileNamed: "/audio/background.mp3")
    ...
    func nodeTapped(name : String) {
        ...
        if name == "ok" {
            ...
            print(gameScene)
            if gameScene == 2{
                sd_air_con.run(SKAction.play())
            }
            if gameScene == 3{
                sd_shower.run(SKAction.play())
            }
            if gameScene == 4{
                sd_ref.run(SKAction.play())
    ...
    func scene_0(name:String) -> String{
        if(name == "ok"){
            ...
            if textField.text == ""{
                textField.text = "Camel"
            }
            return "Good morning," + textField.text! + ".You just get up from bed. You need to turn off the lamp and get the phone call in next room."
        }
        return ""
    }
    ...
    func scene_1(name:String) -> String{
        if(name == "light"){
            ...
            self.run(SKAction.sequence([
                ...
                SKAction.run({
                    self.sd_lamp_switch.run(SKAction.play())
                })
            ]))
            gameScene += 1
    ...
    func scene_2(name:String) -> String{
        if (name == "wind" || name == "aircon"){
            ...
            self.run(SKAction.sequence([
                SKAction.wait(forDuration: 0.1),
                SKAction.run({
                    self.sd_waterer.run(SKAction.play())
                })
            ]))
        ...
        if (name == "shower" || name == "water"){
            ...
            if fire.isHidden{
                self.run(SKAction.sequence([
                    SKAction.wait(forDuration: 0.1),
                    SKAction.run({
                        self.sd_fire_down.run(SKAction.play())
                    })
                ]))
//                sd_fire_down.run(SKAction.play())
            }else{
                self.run(SKAction.sequence([
                    SKAction.wait(forDuration: 0.1),
                    SKAction.run({
                        self.sd_fire.run(SKAction.play())
                    })
                ]))
//                sd_fire.run(SKAction.play())
            }
        }else if(name == "toliet"){
    ...
    func scene_4(name:String) -> String{
        ...
        if (name == "freezer"){
            ...
            if (freezer.texture?.description.contains("02"))! {
                sd_ref.run(SKAction.stop())
            }else{
                sd_ref.run(SKAction.play())
            }
        ...
        if detect && gameScene == 4{
            gameScene += 1
            return "\"" + textField.text! + "! Come out to play!\" Your friend yelled outside."
        }
        return ""
        
    }
    func scene_5(name:String) -> String{
        if (name == "fan" || name == "lamp"){
            self.run(SKAction.sequence([
                SKAction.wait(forDuration: 0.1),
                SKAction.run({
                    self.sd_lamp_switch.run(SKAction.play())
        ...
        if (lamp.texture?.description.contains("02"))! && fan.isPaused && gameScene == 5{
            gameScene += 1
            return "Congratulations! You solve all the task! Now you can feel free to look around in this wonderful world!"
        }
        return ""
    }
...
textField.autoresizingMask = [UIView.AutoresizingMask.flexibleLeftMargin, UIView.AutoresizingMask.flexibleRightMargin, UIView.AutoresizingMask.flexibleTopMargin, UIView.AutoresizingMask.flexibleBottomMargin]
